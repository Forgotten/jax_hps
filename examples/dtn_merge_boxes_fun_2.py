...
box_lvl_0 = [[ cheb.leaf(nx,ny, [x, y]) for x in x_coord] for y in y_coord]
...
for row in box_lvl_0:
    box_lvl_1.append([cheb.box(c, c1) for c, c1 in zip(row[::2], row[1::2])])

box_lvl_2 = []
for col1, col2 in zip(box_lvl_1[::2], box_lvl_1[1::2]):
    box_lvl_2.append([cheb.box(r1, r2) for r1, r2 in zip(col1, col2)])

# the second level of merging
box_lvl_3 = []
for row in box_lvl_2:
    box_lvl_3.append([cheb.box(c, c1) for c, c1 in zip(row[::2], row[1::2])])

box_lvl_4 = []
for col1, col2 in zip(box_lvl_3[::2], box_lvl_3[1::2]):
    box_lvl_4.append([cheb.box(r1, r2) for r1, r2 in zip(col1, col2)])
...
u_bdy = sol(box_lvl_4[0][0].x_grid, box_lvl_4[0][0].y_grid)

m_lvl_0 = len(box_lvl_0)
n_lvl_0 = len(box_lvl_0[0])

w_int_lvl_0, h_bdy_lvl_0 = cheb.solve_leaves(box_lvl_0, rhs_grid)

# first and second levels

(w_int_lvl_1, w_int_lvl_2),\
(h_bdy_lvl_1, h_bdy_lvl_2) = cheb.solve_up_one_level(box_lvl_1, box_lvl_2, h_bdy_lvl_0)

# third and fourth levels

(w_int_lvl_3, w_int_lvl_4),\
(h_bdy_lvl_3, h_bdy_lvl_4) = cheb.solve_up_one_level(box_lvl_3, box_lvl_4, h_bdy_lvl_2)
...
# third and fourth levels
u_bdy_lvl_2 = cheb.solve_down_one_level(box_lvl_2, box_lvl_3, box_lvl_4,
                                        w_int_lvl_3, w_int_lvl_4, u_bdy_lvl_4)

# first and second levels

u_bdy_lvl_0 = cheb.solve_down_one_level(box_lvl_0, box_lvl_1, box_lvl_2,
                                        w_int_lvl_1, w_int_lvl_2, u_bdy_lvl_2)

# solving the interior 

u_int = cheb.solve_int(box_lvl_0, w_int_lvl_0, u_bdy_lvl_0)
